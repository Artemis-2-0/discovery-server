...
# Extract the version below <artifactId>discovery-server</artifactId>
CURRENT_VERSION=$(awk '
    /<artifactId>discovery-server<\/artifactId>/ { found=1 }
    found && /<version>/ {
...
if [[ -z "$CURRENT_VERSION" ]]; then
    echo "Error: Unable to find <version> tag for <artifactId>discovery-server</artifactId> in pom.xml"
    exit 1
fi
...
# Update only the version below <artifactId>discovery-server</artifactId>
